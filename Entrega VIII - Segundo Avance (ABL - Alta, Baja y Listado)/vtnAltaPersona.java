		...
		bot1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					int tel = Integer.parseInt(box8.getText()); // Parseo separado

					if (ci < 10000000 || ci > 99999999) {
						JOptionPane.showMessageDialog(null, "La CI debe tener 8 dígitos", "Error",
								JOptionPane.ERROR_MESSAGE);
						return;
					}

					if (tel <= 0) { // Validación simple para teléfono
						JOptionPane.showMessageDialog(null, "El teléfono debe ser un número válido", "Error",
								JOptionPane.ERROR_MESSAGE);
						return;
					}

					logica.altaPersona(ci, nombre, apellido, tipo, grupo, tel); // Ahora incluye tel
					ventanaPrincipal.actualizar2(); // Refresca
					dispose(); // Cierra la ventana

				} catch (NumberFormatException ex) {
					JOptionPane.showMessageDialog(null, "La CI o teléfono deben ser números válidos", "Error",
							JOptionPane.ERROR_MESSAGE);
				} catch (Exception ex) {
